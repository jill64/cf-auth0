import type { Jwt, Secret } from '../../jsonwebtoken/esm/index.js'
...
declare namespace JwksRsa {
  class JwksClient {
    ...
    getSigningKey(
      ...
      cb: (err: Error | null, key?: SigningKey) => void
    ): void
  }
  ...
  ): SecretCallbackLong & GetVerificationKey
  ...
  function hapiJwt2Key(
    options: HapiJwtOptions
  ): (decodedToken: DecodedToken, cb: HapiCallback) => void
  ...
  function hapiJwt2KeyAsync(
    options: HapiJwtOptions
  ): (decodedToken: DecodedToken) => Promise<{ key: string }>

  function koaJwtSecret(
    options: KoaJwtOptions
  ): (header: TokenHeader) => Promise<string>
